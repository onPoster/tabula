...
import { Dialog, DialogContent, DialogProps, Box, TextField, Typography, List, ListItem, Divider } from "@mui/material"
...
const CommandDialog: React.FC<CommandDialogProps> = ({ children, ...props }) => {
  return (
    <Dialog {...props}>
      <DialogContent>
        <Box sx={{ overflow: "hidden", p: 0, boxShadow: 3 }}>{children}</Box>
      </DialogContent>
    </Dialog>
  )
}

const CommandInput = React.forwardRef<HTMLInputElement, React.ComponentPropsWithoutRef<typeof TextField>>(
  ({ className, ...props }, ref) => (
    <Box sx={{ display: "flex", alignItems: "center", borderBottom: 1, px: 2 }}>
      <svg
        width="469"
        height="469"
        viewBox="0 0 469 469"
...
const CommandList = React.forwardRef<HTMLUListElement, React.ComponentPropsWithoutRef<typeof List>>(
  ({ className, ...props }, ref) => (
    <List ref={ref} sx={{ maxHeight: 300, overflowY: "auto", overflowX: "hidden" }} {...props} />
  ),
)

const CommandEmpty = React.forwardRef<HTMLDivElement, React.ComponentPropsWithoutRef<"div">>((props, ref) => (
  <Box ref={ref} sx={{ py: 6, textAlign: "center", fontSize: "0.875rem" }} {...props}>
    No results found.
  </Box>
))

const CommandGroup = React.forwardRef<HTMLUListElement, React.ComponentPropsWithoutRef<typeof List>>(
  ({ className, ...props }, ref) => (
    <List
      ref={ref}
      sx={{
...
const CommandItem = React.forwardRef<HTMLLIElement, React.ComponentPropsWithoutRef<typeof ListItem>>(
  ({ className, ...props }, ref) => (
    <ListItem
      ref={ref}
      sx={{
...
const CommandSeparator = React.forwardRef<HTMLHRElement, React.ComponentPropsWithoutRef<typeof Divider>>(
  ({ className, ...props }, ref) => (
    <Divider ref={ref} sx={{ mx: -1, height: "1px", backgroundColor: "border" }} {...props} />
  ),
)
...
const CommandShortcut: React.FC<React.HTMLAttributes<HTMLSpanElement>> = ({ className, ...props }) => {
  return (
    <Typography
      component="span"
      sx={{ ml: "auto", fontSize: "0.75rem", letterSpacing: "0.1em", color: "text.secondary" }}
      {...props}
    />
  )
}
